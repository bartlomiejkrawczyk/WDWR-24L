# https://github.com/ralphma1203/trun_mvnt
import math
import numpy as np
...
def imp(a, b):
    ...
    if a == -math.inf or b == math.inf:
        ...
        if a < 0 and b > 0:
            return imp_case2(a, b)
        if a >= 0:
            return imp_case3(a, b)
        if b <= 0:
            return imp_case5(a, b)
...
def imp_acc(a, b):
    ...
    if a == -math.inf or b == math.inf:
        ...
        if a < 0 and b > 0:
            return imp_acc_case2(a, b)
        if a >= 0:
            return imp_acc_case3(a, b)
        if b <= 0:
            return imp_acc_case3(-b, -a)
...
def rtmvn(n=1, Mean=np.zeros(1), Sigma=np.ones(1),
    ...
    if Mean.shape == (1, 1):
        ...
        if np.array(ini).size == p:
            ...
            if bound_check == 0:
                print(
                    'initial is outside or too close from boundary, will be auto-corrected by MP-inverse!')
        elif bound_check == 0:
    ...
    for i in range((thin+1)*n+burn):
        for j in range(p):
            ...
            # m by p-1 matrix by removing the jth column of R
            Rj = np.delete(R, j, 1)
            # p-1 vector by removing the jth element
            zj = np.delete(z, j).reshape((p-1, 1))
            ...
            if pos.sum() == 0:
                ...
                # when r_jk>0, a associated with lower bound
                lower_pos = (a_temp[pos]/rj[pos]).max()
                # b                 upper
                upper_pos = (b_temp[pos]/rj[pos]).min()
            ...
            if neg.sum() == 0:
                ...
                # when r_jk<0, a                 upper
                upper_neg = (a_temp[neg]/rj[neg]).min()
                # b                 lower
                lower_neg = (b_temp[neg]/rj[neg]).max()
...
def rtmvt(n=1, Mean=np.zeros(1), Sigma=np.ones(1), nu=100,
    ...
    # initial value for the transformed tmvt
    x = np.linalg.solve(Sigma_chol, ini - Mean)
